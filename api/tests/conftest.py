...
def global_mocks(monkeypatch):
    ...
    monkeypatch.setattr("app.services.ai_services.chat_service.get_db", lambda: {"users": MagicMock(), "lab_reports": MagicMock(), "health_data": MagicMock()})
    monkeypatch.setattr("app.services.ai_services.lab_report_service.get_db", lambda: {"lab_reports": MagicMock()})
    monkeypatch.setattr("app.services.ai_services.lab_report_service.get_db", lambda: {"lab_reports": MagicMock()})
    monkeypatch.setattr("app.services.ai_services.lab_report_service.get_lab_report_service", lambda: MagicMock())
