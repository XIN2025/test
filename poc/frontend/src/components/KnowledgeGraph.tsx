...
export const KnowledgeGraph: React.FC = () => {
  ...
  const fetchGraphData = async () => {
    ...
    try {
      const response = await axios.get<ApiGraphData>(
      ...
      const data: GraphData = {
        ...
        links: deduplicateLinks(
          response.data.links.map((link) => ({
            ...link,
            source:
              response.data.nodes.find((node) => node.id === link.source) ||
              link.source,
            target:
              response.data.nodes.find((node) => node.id === link.target) ||
              link.target,
          }))
        ),
      };
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent bg={bgColor}>
            ...
            <AlertDialogBody>
              Are you sure? This will permanently delete all nodes and
